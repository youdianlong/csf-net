from networks.unet import UNet, MCNet2d_v1, MCNet2d_v2, MCNet2d_v3, ours_net
from networks.VNet import VNet, MCNet3d_v1, MCNet3d_v2,ours_net_3d
...
def net_factory(net_type="unet", in_chns=1, class_num=4, mode="train"):
    ...
    elif net_type == "ours_net":
        net = ours_net(in_chns=in_chns, class_num=class_num).cuda()
    elif net_type == "ours_net_3d" and mode == 'train':
        net = ours_net_3d(n_channels=in_chns, n_classes=class_num, normalization='batchnorm', has_dropout=True).cuda()
    elif net_type == "ours_net_3d" and mode == 'test':
        net = ours_net_3d(n_channels=in_chns, n_classes=class_num, normalization='batchnorm', has_dropout=False).cuda()
